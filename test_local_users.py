...
selected = client.post(BASE + "/api/session", json={"user_id": operator["id"]})
...
duplicate = client.post(
    BASE + "/api/users",
    json={"username": f"{PREFIX}_operator", "display_name": "Duplicate", "role": "owner"},
)
...
assert "Password hashing and session expiry are scheduled for Day 62" in html
