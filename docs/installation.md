...
To verify a clean installation, use a new directory or machine, clone the repository again, create a new virtual environment, install `requirements-dev.txt`, run the full test commands, start the application and confirm that `/healthz` returns HTTP 200 with schema versions `[1, 2]`. Do not copy an existing `data/` directory into the test because that would hide migration and first-start problems.
