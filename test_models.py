...
with tempfile.NamedTemporaryFile(suffix=".sqlite3") as handle:
    ...
    conn.execute(
        ...
        ("job-unit", "sample.png", "/tmp/sample.png", "abc123", "READY", "2026-01-01", "2026-01-01"),
    )
    ...
    checkpoint = conn.execute("SELECT y_mm,evidence,confidence FROM checkpoints WHERE job_id=?", ("job-unit",)).fetchone()
    ...
    event_payload = json.loads(conn.execute("SELECT payload FROM events WHERE job_id=?", ("job-unit",)).fetchone()[0])
    ...
    decision = conn.execute("SELECT recommendation,mode,operator_action FROM decisions WHERE job_id=?", ("job-unit",)).fetchone()
