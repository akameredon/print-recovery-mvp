...
assert health_body["checks"]["database"]["schema_versions"] == [1, 2]
...
diagnostics = requests.get(BASE + "/api/diagnostics", headers={"X-Correlation-ID": "day5-diagnostics-test"})
...
print({"status": "passed", "health": health_body["status"], "schema_versions": health_body["checks"]["database"]["schema_versions"]})
